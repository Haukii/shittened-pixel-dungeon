...
import com.shatteredpixel.shatteredpixeldungeon.sprites.DM249Sprite;
...
public class DM249 extends Mob implements Callback {

	private static final float TIME_TO_FIRE	= 2f;

	{
		spriteClass = DM249Sprite.class;
	...
	protected boolean doAttack( Char enemy ) {
		...
		if (Dungeon.level.adjacent( pos, enemy.pos )
			...
			if (hit( this, enemy, false )) {
				float delay = 0.11f;
				for (int i = 0; i < 8; i++) { //TODO LEARN HOW CALLBACKS WORK
					if (!enemy.isAlive()) {
						Dungeon.fail(this);
					...
					Timer.schedule(new Timer.Task(){
						@Override
						public void run(){
							int dmg = enemy.defenseProc(DM249.this, Random.NormalIntRange(0,1));
							dmg = Math.round(dmg * AscensionChallenge.statModifier(DM249.this));
							enemy.damage(dmg, DM249.this);
