	...
	public void burn() {
		...
		for (Item item : items.toArray( new Item[0] )) {
			if (item instanceof Scroll && !item.unique) {
				...
				burnt = true;
			} else if (item instanceof SmallMeat || item instanceof SmallFrozenCarpaccio) {
				replace(item, ChargrilledSmallMeat.cook(item.quantity));
				burnt = true;
	...
	public void freeze() {
		...
		for (Item item : items.toArray( new Item[0] )) {
			if (item instanceof MysteryMeat) {
				...
				frozen = true;
			}else if (item instanceof SmallMeat) {
				replace( item, SmallFrozenCarpaccio.cook( (SmallMeat)item ) );
				frozen = true;
