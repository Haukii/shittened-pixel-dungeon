...
//Literally just Lightning.java with different sprite
public class Gunshot extends Group {
	...
	public Gunshot(int from, int to, Callback callback){
		this(Arrays.asList(new Arc(from, to)), callback);
	}

	public Gunshot(PointF from, int to, Callback callback){
		this(Arrays.asList(new Arc(from, to)), callback);
	}

	public Gunshot(int from, PointF to, Callback callback){
		this(Arrays.asList(new Arc(from, to)), callback);
	}

	public Gunshot(PointF from, PointF to, Callback callback){
		this(Arrays.asList(new Arc(from, to)), callback);
	}

	public Gunshot(List<Arc> arcs, Callback callback ) {
...
//		Blending.setLightMode();
	...
	public static class Arc extends Group {
		...
		public Arc(PointF from, PointF to){
			...
			arc1 = new Image(Effects.get(Effects.Type.GUNSHOT));
			arc1.x = start.x - arc1.origin.x;
			arc1.y = start.y - arc1.origin.y;
			arc1.origin.set( 0, arc1.height()/2 );
			add( arc1 );

			arc2 = new Image(Effects.get(Effects.Type.GUNSHOT));
		...
		@Override
		public void update() {
			float x2 = (start.x + end.x) / 2 + Random.Float( -0.8f, + 0.8f );
			float y2 = (start.y + end.y) / 2 + Random.Float( -0.8f, + 0.8f );
